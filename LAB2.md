...
Для создания экземпляра класса, который реализует `MutableWorker` можно воспользоваться его статическим методом `of`, который для вас уже реализован. Чтобы так можно было создать экземпляр, у вашего рабочего должен быть статический (фабричный) метод вида:
```
public static MyWorker of(String confPath) {
