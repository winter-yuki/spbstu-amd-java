...
- Тогда конвертация в коде ваглядит так (жутко не эффективно, но для простоты - сойдет):
...
5. Возвращает количество элементов исходной коллекции `data`, которое удалось обработат (т.е. ```((byte[]) data).length```, например, если пришли данные в формате `byte[]` и удалось обработать все). 
    - Но не будем четко специфицировать семантику возвращаемого значения. О его роли договоритесь с товарищами, с которыми планируете обмениваться `Worker`ами.
...
Можно было бы создавать рабочих и через конструктор, но код, в котором объекты создаются не напрямую, а через фабрики, по праву считается значительно более гибким.
